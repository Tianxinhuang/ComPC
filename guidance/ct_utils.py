...
class StableDiffusion(nn.Module):
    ...
    def train_step(
        ...
        depth_map = depth_map.to(self.dtype)
        ...
        with torch.no_grad():
            ...
            #print(noise.shape, latents.shape,t.shape)
            latents_noisy = self.scheduler.add_noise(latents, noise, t)
            #print(latents_noisy.shape)
            # pred noise
            latent_model_input = torch.cat([latents_noisy] * 2)
            #print(latent_model_input.shape, latents.shape, latents_noisy.shape)
            #assert False
            tt = torch.cat([t] * 2)
            ...
            for i in range(self.num_train_timesteps):
                keeps = [
                    1.0 - float(i / self.num_train_timesteps < s or (i + 1) / self.num_train_timesteps > e)
            ...
            if guess_mode:
                ...
                controlnet_prompt_embeds = embeddings.chunk(2)[1]
            else:
            ...
            #print(control_model_input.shape, controlnet_prompt_embeds.shape, pred_rgb_512.shape, t.shape)
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                    control_model_input,
                    t,
                    encoder_hidden_states=controlnet_prompt_embeds,
                    controlnet_cond=depth_map.unsqueeze(0),
            ...
            # grad = grad.clamp(-1, 1)
            #print(latents.shape, grad.shape)
            #assert False
