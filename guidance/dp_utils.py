...
class StableDiffusion(nn.Module):
    def __init__(
        ...
        self.unet = pipe.unet
        self.prepare_depth_map = pipe.prepare_depth_map
        self.image_processor = pipe.image_processor
        self.prepare_latents = pipe.prepare_latents
        #self.get_timesteps = pipe.get_timesteps
    ...
    def train_step(
        ...
        if as_latent:
            ...
            #image = self.image_processor.preprocess(pred_rgb)
            #latents = self.prepare_latents(image, latent_timestep, batch_size, 1, prompt_embeds.dtype, self.device, generator)
        else:
            # interp to 512x512 to be fed into vae.
            pred_rgb_512 = F.interpolate(pred_rgb, (512, 512), mode="bilinear", align_corners=False)
            depth_map = F.interpolate(depth_map.unsqueeze(0), (512, 512), mode="bilinear", align_corners=False).squeeze(0)
            # encode image into latents with vae, requires grad!
            latents = self.encode_imgs(pred_rgb_512)
        image = self.image_processor.preprocess(pred_rgb_512)

        #depth_mask = self.prepare_depth_map(pred_rbg,depth_map,
        #    batch_size,
        #    True,
        #    self.embeddings['pos'].dtype,
        #    self.device,
        #)#pred_rgb or depth
        # 6. Set timesteps
        #self.scheduler.set_timesteps(self.num_train_timesteps, device=self.device)
        #print(self.num_train_timesteps)
        #timesteps, num_inference_steps = self.get_timesteps(self.num_train_timesteps, 0.7, self.device)
        #latent_timestep = timesteps[:1].repeat(batch_size * 1)
        #print(latent_timestep)
        #latents = self.prepare_latents(image, latent_timestep, batch_size, 1, self.embeddings['pos'].dtype, self.device, generator)
        ...
        with torch.no_grad():
            ...
            #latents = self.prepare_latents(image, t, batch_size, 1, self.embeddings['pos'].dtype, self.device, generator)
            #print(latents.shape)
            ...
            depth_mask = self.prepare_depth_map(image,depth_map,
            batch_size,
            True,
            self.embeddings['pos'].dtype,
            self.device,
            ...
            #print(latent_model_input.shape, depth_mask.shape)
            #latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
            #print(latent_model_input.shape, depth_mask.shape)

            latent_model_input = torch.cat([latent_model_input, depth_mask], dim=1)
            #print(latent_model_input.shape, embeddings.shape)
            #assert False
